...
//
PROGMEM const char deviceDescrMIDI[18] = {	/* USB device descriptor */
...
PROGMEM const char configDescrMIDI[101] = {	/* USB configuration descriptor */
...
// the standard AC interface descriptor and the class-specific AC interface descriptor 
...
uchar usbFunctionSetup(uchar data[8]) {
    /* Handling class-specific requests would be done here
    ...
    return USB_NO_MSG; // call usbFunctionRead if needed
}

uchar usbFunctionRead(uchar * data, uchar len) {
    // Send zeroes if someone asks!
    data[0] = 0;
    data[1] = 0;
    data[2] = 0;
    data[3] = 0;
